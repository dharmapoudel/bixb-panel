package com.dharmapoudel.bxpanel.core
...
import com.dharmapoudel.bxpanel.PanelActivity
import com.dharmapoudel.bxpanel.Preferences
import com.dharmapoudel.bxpanel.actions.Option
import com.dharmapoudel.bxpanel.actions.TorchManager
import java.util.*
import kotlin.concurrent.timerTask


class BxPanelAccessibilityService : AccessibilityService() {
    companion object {
    ...
    private fun init() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            vibrationEffect = VibrationEffect.createOneShot(VIBE_DURATION, VibrationEffect.DEFAULT_AMPLITUDE)
        }
    }
    ...
    override fun onKeyEvent(event: KeyEvent?): Boolean {
        if (event == null || !prefs.isEnabled || audioManager?.mode != AudioManager.MODE_NORMAL) return false
        ...
        when (event.action) {
            KeyEvent.ACTION_DOWN -> {
                if (event.eventTime - doubleUpTime < DOUBLE_PRESS_DURATION) {
                    doubleUpTask?.cancel()
                    doubleUpFlag = true
                    doubleSelect(prefs.doubleUpMode, -1)
    ...
    private fun doubleSelect(mode: Int, index: Int){
        Intent(applicationContext, PanelActivity::class.java).also {
